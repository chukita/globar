...
6. **Se cobra primero y se factura después** (rediseño de sept. 2026, commit `d6971c2`). Flujo mensual, todo desde `/admin/liquidaciones` (server actions en `src/lib/liquidaciones-actions.ts`):
   ...
   2. Desde esa fecha el revendedor tiene **`configuracion.mesesGraciaFactura` meses** (default 3, snapshoteado en `liquidaciones.facturaVenceEn`) para subir su factura en PDF a nombre de Grupo Globaliza desde `/panel/facturas`.
