...
6. **Se cobra primero y se factura después** (rediseño de sept. 2026, commit `d6971c2`). Flujo mensual, todo desde `/admin/liquidaciones` (server actions en `src/lib/liquidaciones-actions.ts`):
   ...
   4. **Bloqueo**: si un revendedor tiene una liquidación vencida (`facturaVenceEn < now`) cuya factura todavía no está **aprobada** — sea `pagada` (no subió nada) o `en_revision` (subió, falta aprobarla) — queda excluido de la liquidación siguiente hasta ponerse al día. Las comisiones se le siguen acumulando igual y las cobra todas juntas cuando se destraba. El preview del admin distingue el motivo ("factura vencida sin enviar" vs "factura en revisión sin aprobar").
   5. **Recordatorios automáticos** (`src/lib/recordatorios-liquidacion.ts` + cron diario, ver más abajo): aviso suave 30 días antes del vencimiento y aviso de bloqueo al vencer, solo a las que están en `pagada` (el revendedor todavía no subió nada). Una `en_revision` vencida es problema del admin, la ve marcada en su panel.
