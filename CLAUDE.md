...
6. **Se cobra primero y se factura después** (rediseño de sept. 2026, commit `d6971c2`). Flujo mensual, todo desde `/admin/liquidaciones` (server actions en `src/lib/liquidaciones-actions.ts`):
   1. A principio de mes el superadmin **liquida**: `confirmarLiquidacionAction` agrupa todas las cuotas `generada` del revendedor acumuladas hasta fin del mes anterior en una fila de `liquidaciones`, transfiere la plata a mano por Mercado Pago y sube el comprobante. Las cuotas pasan a `liquidada`.
   2. El revendedor tiene que subir su factura en PDF desde `/panel/facturas` (por el monto exacto de la liquidación) **antes de la liquidación del mes siguiente** — si queda pendiente, no entra en ese pago (punto 4). `configuracion.mesesGraciaFactura` (default 1) solo se snapshotea en `liquidaciones.facturaVenceEn` para el ritmo de los recordatorios (punto 5), ya no es un plazo duro. `/panel/facturas` incluye una **guía paso a paso** para emitirla en AFIP Comprobantes en Línea, precalculada para ese pago (`FacturaGuia`, `src/components/FacturaGuia.tsx`). Los datos del receptor viven en **`DATOS_FISCALES` (`src/lib/constants.ts`)** — nombre legal (`Carlos Hugo Costantino`), nombre comercial (`Grupo Globaliza`), CUIT `20-29739072-5`, condición IVA `Responsable Monotributo`, tipo `Factura C`; se muestran en el panel de facturas y en los mails `emailLiquidacionPagada` / `emailRecordatorioFacturaPendiente` / `emailFacturaLiquidacionRechazada` (`src/lib/email.ts` → `datosFacturaHtml`). No se incluye el domicilio a propósito: ARCA lo completa del padrón al cargar el CUIT.
   3. Al subir el PDF la liquidación queda **`en_revision`** — NO se cierra nada, las cuotas siguen en `liquidada`. El superadmin la revisa en `/admin/liquidaciones`:
      - **`aprobarFacturaLiquidacionAction`** → liquidación `facturada` (terminal) y recién ahí las cuotas pasan a `pagada`.
      - **`rechazarFacturaLiquidacionAction`** (motivo obligatorio) → vuelve a `pagada`, se limpia el `facturaUrl`, se guarda `facturaRechazoMotivo` y se le avisa al revendedor. Tiene que subir una nueva; el `facturaVenceEn` no se reinicia.
   4. **Bloqueo**: si un revendedor tiene **cualquier** liquidación anterior cuya factura no está **aprobada** — sea `pagada` (no subió nada) o `en_revision` (subió, falta aprobarla) — queda excluido de la liquidación siguiente hasta ponerse al día. **No importa `facturaVenceEn`**: el pago es mensual (cae cualquier día del 1 al 10) y no debe quedar ninguna factura pendiente para habilitar el siguiente. Las comisiones se le siguen acumulando y se pagan **todas juntas** en la primera liquidación después de regularizar. El preview del admin distingue el motivo ("factura pendiente sin enviar" vs "factura en revisión sin aprobar"). Chequeado tanto en el preview (`getPreviewLiquidacionMesAnterior`) como en `confirmarLiquidacionAction`.
   5. **Recordatorios automáticos** (`src/lib/recordatorios-liquidacion.ts` + cron diario, ver más abajo): sobre las liquidaciones en `pagada`, aviso suave `RECORDATORIO_SUAVE_DIAS_ANTES` (10) días antes de `facturaVenceEn` y aviso de bloqueo al pasar esa fecha. `facturaVenceEn` ya **no gatea el bloqueo** (punto 4), solo marca el ritmo de estos recordatorios. Una `en_revision` es problema del admin, la ve marcada en su panel.

   (El `facturas`/`cuotas_facturas` legacy y `/admin/facturas` son del flujo viejo — factura antes de cobrar —; no se escriben más, se conservan para histórico.)
