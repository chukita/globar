...
export function CapacitacionClient({
  ...
  const QUIZ = [
    ...
    },
    {
      ...
      options: [
        `Un único pago de ${fmtARS(comisionMonto * comisionMeses)}`,
        `${fmtARS(comisionMonto)} por mes durante ${comisionMeses} meses`,
        "El 10% de por vida",
      ],
      correct: 1,
    },
  ...
  function submitQuiz() {
    const allOk = QUIZ.every((q) => answers[q.id] === q.correct);
